...
namespace RPMHSI
{
    public class MLS : InternalModule
    {
        ...
        public bool DrawMLS(RenderTexture screen, float aspectRatio)
        {
            ...
            double bearing = NavUtilities.Utils.CalcBearingToBeacon(thisVessel, Rwy);
            double dme = NavUtilities.Utils.CalcDistanceToBeacon(thisVessel, Rwy);
            double elevation = NavUtilities.Utils.CalcElevationAngle(thisVessel, Rwy);
            float locDeviation = NavUtilities.Utils.CalcLocalizerDeviation(bearing, Rwy);
            float gsDeviation = NavUtilities.Utils.CalcGlideslopeDeviation(elevation, glideSlope);
            ...
            if (dme < 30000)
            {
                ...
                var fB = NavUtilities.Utils.CalcBearingTo(Utils.CalcRadiansFromDeg(Rwy.gsLongitude-thisVessel.longitude),
        ...
        private byte inBeaconArea(float locDeviation, Vessel v, Runway r)
        {
            ...
            {
                var gcd = NavUtilities.Utils.CalcGreatCircleDistance(v.latitude,v.longitude,r.gsLatitude,r.gsLongitude, r.body);
        ...
        private static void drawCenterRotatedImage(float headingDeg, Vector2 centerPercent, Material mat, RenderTexture screen, float xOffset, float yOffset)
        {
            ...
            headingDeg = (float)NavUtilities.Utils.makeAngle0to360(headingDeg);
            headingDeg = (float)NavUtilities.Utils.CalcRadiansFromDeg(headingDeg);
        ...
        public void ButtonProcessor(int buttonID)
        {
            ...
            if (buttonID == 2) //print coordinates on Debug console
            {
                ...
                Debug.Log("Lat: " + FlightGlobals.ship_latitude + " Lon: " + FlightGlobals.ship_longitude + " GCD: " + NavUtilities.Utils.CalcGreatCircleDistance(v.latitude, v.longitude, r.gsLatitude, r.gsLongitude, r.body));
            }
        ...
        public void Start()
        {
            rwyList = NavUtilities.ConfigLoader.GetRunwayListFromConfig(settingsFileURL);
            gsList = NavUtilities.ConfigLoader.GetGlideslopeListFromConfig(settingsFileURL);
            ...
            if (folder.Exists)
            {
                ...
                {
                    rwyList.AddRange(NavUtilities.ConfigLoader.GetRunwayListFromConfig("GameData/KerbalScienceFoundation/NavInstruments/Runways/" + finfo.Name));
                }
            ...
            Byte[] arrBytes;
            arrBytes = KSP.IO.File.ReadAllBytes<MLS>("overlay.png");
            hsi_overlay.LoadImage(arrBytes);

            arrBytes = KSP.IO.File.ReadAllBytes<MLS>("large_heading_card.png");
            hsi_heading_card.LoadImage(arrBytes);

            arrBytes = KSP.IO.File.ReadAllBytes<MLS>("gs_pointer.png");
            hsi_GS_pointer.LoadImage(arrBytes);

            arrBytes = KSP.IO.File.ReadAllBytes<MLS>("hsi_back.png");
            hsi_back.LoadImage(arrBytes);

            arrBytes = KSP.IO.File.ReadAllBytes<MLS>("NDB_needle.png");
            hsi_NDB_needle.LoadImage(arrBytes);

            arrBytes = KSP.IO.File.ReadAllBytes<MLS>("course_needle.png");
            hsi_course_needle.LoadImage(arrBytes);

            arrBytes = KSP.IO.File.ReadAllBytes<MLS>("course_deviation_needle.png");
            hsi_course_deviation_needle.LoadImage(arrBytes);

            arrBytes = KSP.IO.File.ReadAllBytes<MLS>("markerIndicator.png");
            hsi_mkr_bcn.LoadImage(arrBytes);

            arrBytes = KSP.IO.File.ReadAllBytes<MLS>("flags.png");
